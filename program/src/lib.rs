...
fn process_instruction(
    ...
    info!("Prediction Market Rust program entrypoint");
    ...
    // let marketname = LittleEndian::read_u32(&data[0..10]);

    // // 2nd 10 bytes = description
    let marketdescription = LittleEndian::read_u32(&data[10..20]);

    // // 3rd 10 bytes = category
    let marketcategory = LittleEndian::read_u32(&data[20..30]);

    // // 4th 10 bytes = votes can be - x to + x
    let uservote = LittleEndian::read_u32(&data[40..50]);
    ...
    // Increment and store the number of times the user joined prediction market
    let mut data = account.try_borrow_mut_data()?;
    let mut marketvotes = LittleEndian::read_u32(&data);
    appvotes += 1;
    LittleEndian::write_u32(&mut data[0..], appvotes);

    info!("Saved prediction market!");
